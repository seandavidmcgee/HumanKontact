...
class PagingViewController0_0: UIViewController, UITextFieldDelegate {
    ...
    func keyPressed(sender: UIButton!) {
        ...
        self.refreshData()
        for index in 0..<objects.count {
            buttonsArray[index].setTitle("\((sender.titleLabel?.text)! + (objects[index] as! String))", forState: UIControlState.Normal)
            buttonsArray[index].titleLabel!.text = "\((sender.titleLabel?.text)! + (objects[index] as! String))"
        }
